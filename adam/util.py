...
def omit(d: Dict, keys: Iterable) -> Dict:
    ...
    return dict((k, v) for k, v in d.items() if k not in as_set(keys))


def d_subset(d: Dict, keys: Iterable) -> Dict:
    """Returns copy of 'd' with only members of 'keys'; all other items are
    omitted."""
    return dict((k, v) for k, v in d.items() if k in as_set(keys))
...
def tupdict(**kwargs) -> Tuple[Tuple[str, Hashable]]:
    return tuple((k, v) for k, v in kwargs.items())  # type: ignore
...
def intersection(*iters: Iterable) -> Set:
    ...
    sets = [s if isinstance(s, set) else set(s) for s in iters]
    if sets:
...
# TODO UT
def instantiate_dataclass_from_kwargs(dclass: Type[T], kwargs: Dict[str, Any]) -> T:
...
def as_dstr(x: Union[Dict, Any, None, Collection[Tuple[str, Hashable]]]) -> str:
...
def asdict_with_classvars(x) -> Dict[str, Any]:
    ...
    definition."""
    return dict((name, getattr(x, name)) for name in x.__dataclass_fields__)
    ...
    try:
        ...
        if safe_issubclass(origin, type):
            ...
            if not isinstance(o, tuple):
                ...
                return all(is_type_instance(elem, arg) for elem, arg in zip(o, args))
...
class DescStats:
    ...
    def from_data(cls, data: Iterable[Numeric]) -> DescStats:
        ...
        return DescStats(
            fmean(data), stdev(data), len(data), median(data), min(data), max(data)
        )
...
def write_dict_to_csv(
    self, d: Dict, mode: str, prefix: Optional[Sequence] = None, filename: str = "a.csv"
) -> None:
...
def repr_str(name, items):
    ...
    if len(items) == 1:
        ...
        return "%s(%s)" % (name, ", ".join("%s=%s" % (k, nrepr(v)) for k, v in items))
